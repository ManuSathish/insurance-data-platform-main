...
# MAGIC SHOW TABLES IN insurance_catalog_manu.silver;
...
for t in silver_tables:
    count = spark.table(f"insurance_catalog_manu.silver.{t.tableName}").count()
    # Count valid rows: filter where _is_valid = True
    valid = spark.table(f"insurance_catalog_manu.silver.{t.tableName}").filter("_is_valid = true").count()
