...
# MAGIC 1. The `insurance_catalog_manu` must be created FIRST via the Databricks UI:
# MAGIC    Catalog Explorer → + Add → Add a catalog → Name: `insurance_catalog_manu`
# MAGIC 2. You must have CREATE SCHEMA privilege on `insurance_catalog_manu`
...
spark.sql("USE CATALOG insurance_catalog_manu")
print("Using catalog: insurance_catalog_manu")
...
# MAGIC CREATE SCHEMA IF NOT EXISTS insurance_catalog_manu.bronze
...
# MAGIC CREATE SCHEMA IF NOT EXISTS insurance_catalog_manu.silver
...
# MAGIC CREATE SCHEMA IF NOT EXISTS insurance_catalog_manu.gold
...
spark.sql("SHOW SCHEMAS IN insurance_catalog_manu").show()
...
bronze_tables = spark.sql("SHOW TABLES IN insurance_catalog_manu.bronze").collect()
...
silver_tables = spark.sql("SHOW TABLES IN insurance_catalog_manu.silver").collect()
...
for layer, schema in [("BRONZE", "bronze"), ("SILVER", "silver"), ("GOLD", "gold")]:
    tables = spark.sql(f"SHOW TABLES IN insurance_catalog_manu.{schema}").collect()
    print(f"\n{layer} Layer ({len(tables)} tables):")
    for t in tables:
        count = spark.table(f"insurance_catalog_manu.{schema}.{t.tableName}").count()
...
# MAGIC FROM insurance_catalog_manu.gold.mart_policy_360
